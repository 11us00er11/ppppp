...
class DiaryEntry {
  ...
  factory DiaryEntry.fromJson(Map<String, dynamic> j) {
    DateTime parseDT(dynamic v) => v == null ? DateTime.now() : DateTime.parse(v.toString()).toLocal();
    return DiaryEntry(
...
class DiaryService {
  static Future<List<DiaryEntry>> fetch({
    ...
    final uri = Uri.parse('$baseUrl/api/diary').replace(queryParameters: qp);
  ...
  static Future<DiaryEntry> create({
    ...
    // 서버가 생성된 레코드를 통째로 주는 형태를 가정. 필요시 적절히 맞추세요.
    return DiaryEntry.fromJson(j is Map<String, dynamic> ? j : (j['item'] as Map<String, dynamic>));
  }
...
class _HistoryScreenState extends State<HistoryScreen> {
            ...
            children: [
              ...
              Wrap(
                ...
                children: kMoodOptions.map((m) => ChoiceChip(
                  ...
                  selectedColor: moodColor(m).withOpacity(0.25),
                  onSelected: (_) { selectedMood = m; (ctx as Element).markNeedsBuild(); },
                )).toList(),
              ),
              const SizedBox(height: 12),
              TextField(
                  ...
                  Expanded(child: ElevatedButton(
                    onPressed: () async {
                      try {
                        ...
                        if (mounted) Navigator.pop(ctx, created.mood ?? '');
                        // 상단에 바로 반영
  ...
  Future<void> _openFilter() async {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: OutlinedButton.icon(
                        ...
                        label: Text(from == null ? '시작일' : fmtYMD(from!)),
                      ...
                      Expanded(child: OutlinedButton.icon(
                        icon: const Icon(Icons.date_range),
                        label: Text(to == null ? '종료일' : fmtYMD(to!)),
                        onPressed: () async {
                          final now = DateTime.now();
                          final picked = await showDatePicker(
                            context: context,
                  ...
                  Row(
                    children: [
                      TextButton(
                        onPressed: () { setS(() { qCtrl.text=''; from=null; to=null; moods.clear(); }); },
    ...
    // 적용
    setState(() { _query = qCtrl.text.trim().isEmpty ? null : qCtrl.text.trim(); _from = from; _to = to; _moods
      ..clear()..addAll(moods); });
    _load(refresh: true);
  }
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, i) {
            ...
            return Dismissible(
              ...
              confirmDismiss: (_) async { await _deleteEntry(e); return false; },
              child: ListTile(
                ...
                onTap: () {
                  ...
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      title: Row(
                        children: [
                          Container(width: 10, height: 10, decoration: BoxDecoration(color: moodColor(e.mood), shape: BoxShape.circle)),
                          const SizedBox(width: 8),
                          Text(e.mood ?? '기록')
                        ],
                      ),
                      ...
                      actions: [
                        ...
                        TextButton(onPressed: () { Navigator.pop(ctx); _deleteEntry(e); }, child: const Text('삭제')),
                      ],
                    ),
                  );
                },
